...
public interface NetworkInterface {
    ...
    /*@ public instance model non_null PersonInterface[] persons;
      ...
      @ public instance model non_null MessageInterface[] messages; // TODO HashMap<Integer, MessageInterface>
      @ public instance model non_null int[] emojiIdList; // TODO HashMap<Integer(id),Integer(heat)>
    ...
    /*@ public normal_behavior
      ...
      @                                       (message instanceof ForwardMessageInterface) &&
      @                                       containsArticle(((ForwardMessageInterface) message).getArticleId()) &&
      @                                       !(message.getPerson1().getReceivedArticles().contains(((ForwardMessageInterface) message).getArticleId())); // TODO Person.containsArticle
    ...
    /*@ public normal_behavior
      ...
      @ ensures !containsMessage(id); // TODO delete from messages
      @ ensures \old(getMessage(id)).getPerson1().getSocialValue() ==
      @         \old(getMessage(id).getPerson1().getSocialValue()) + \old(getMessage(id)).getSocialValue() &&
      @         \old(getMessage(id)).getPerson2().getSocialValue() ==
      @         \old(getMessage(id).getPerson2().getSocialValue()) + \old(getMessage(id)).getSocialValue(); // TODO add person1,person2 socialValue
      ...
      @         \old(getMessage(id).getPerson2().getMoney()) + ((RedEnvelopeMessageInterface)\old(getMessage(id))).getMoney()); // TODO if(RedEnvelop) person1 - money, person2 + money
      ...
      @         \old(getMessage(id)).getPerson2().getReceivedArticles().size() == \old(getMessage(id).getPerson2().getReceivedArticles().size()) + 1); // TODO if(Forward) person2.receiveArticle(articleId)
      ...
      @         emojiHeatList[i] == \old(emojiHeatList[i]) + 1); // TODO if(Emoji) emojiHeat++
      @ ensures (!(\old(getMessage(id)) instanceof EmojiMessageInterface)) ==> \not_assigned(emojiHeatList);
      @ ensures (\forall int i; 0 <= i && i < \old(getMessage(id).getPerson2().getMessages().size());
      @          \old(getMessage(id)).getPerson2().getMessages().get(i+1) == \old(getMessage(id).getPerson2().getMessages().get(i)));
      @ ensures \old(getMessage(id)).getPerson2().getMessages().get(0).equals(\old(getMessage(id)));
      @ ensures \old(getMessage(id)).getPerson2().getMessages().size() == \old(getMessage(id).getPerson2().getMessages().size()) + 1; // TODO person2.messages[0] = getMessage(id)
      @ also
      @ public normal_behavior
      ...
      @ ensures !containsMessage(id)  // TODO delete from messages
      @ ensures \old(getMessage(id)).getPerson1().getSocialValue() ==
      @         \old(getMessage(id).getPerson1().getSocialValue())+ \old(getMessage(id)).getSocialValue(); // TODO add person1 socialValue
      @ ensures (\forall PersonInterface p; \old(getMessage(id)).getTag().hasPerson(p); p.getSocialValue() ==
      @         \old(p.getSocialValue()) + \old(getMessage(id)).getSocialValue()); // TODO add tag.allPerson socialValue
      ...
      @           p.getMoney() == \old(p.getMoney()) + i)); // TODO if(RedEnvelop && tag.size > 0) i = message.money / tag.size, person1.money -= message.money, tag.allPerson.money += i
      ...
      @         (\forall PersonInterface p; \old(getMessage(id)).getTag().hasPerson(p); p.getReceivedArticles().size() == \old(p.getReceivedArticles().size()) + 1); // TODO if(Forward && tag.size > 0) tag.allPerson.receiveArticle(articleId)
      @ ensures (\old(getMessage(id)) instanceof EmojiMessageInterface) ==>
      @         (\exists int i; 0 <= i && i < emojiIdList.length && emojiIdList[i] == ((EmojiMessageInterface)\old(getMessage(id))).getEmojiId();
      @          emojiHeatList[i] == \old(emojiHeatList[i]) + 1); // TODO if(Emoji) emojiHeat++
      ...
      @ ensures (\forall PersonInterface p; \old(getMessage(id)).getTag().hasPerson(p); p.getMessages().size() == \old(p.getMessages().size()) + 1); // TODO tag.allPerson.messages[0] = getMessage(id) (receiveMessage();)
      @ also
      @ public exceptional_behavior
    ...
    /*@ public normal_behavior // TODO emojis.put(id, 0)
    ...
    /*@ public normal_behavior
      ...
      @          (\num_of int i; 0 <= i && i < \old(emojiIdList.length); \old(emojiHeatList[i] >= limit)); //TODO delete emoji.heat < limit from emojis
      ...
      @          (\old(messages[i]) instanceof EmojiMessageInterface) ==>
      @           (containsEmojiId(\old(((EmojiMessageInterface)messages[i]).getEmojiId())))); // TODO delete EmojiMessage which has beDeletedEmojiId
